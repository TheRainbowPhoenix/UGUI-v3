        ...
        if (active_wnd) {
          ...
          case KEYCODE_EXE:
            if (active_wnd->focused_obj) {
              if (active_wnd->focused_obj->type == OBJ_TYPE_INPUT_FIELD) {
                // Show keyboard for input fields
                UG_OSKeyboard_Show();
              } else if (active_wnd->focused_obj->type == OBJ_TYPE_SLIDER) {
                UG_SLIDER *sld = (UG_SLIDER *)(active_wnd->focused_obj->data);
                ...
                UG_SliderSetValue(&window_1, active_wnd->focused_obj->id,
                                  current_value + step);
              } else {
                // Handle other objects as before
                active_wnd->focused_obj->event = OBJ_EVENT_PRESSED;
                _UG_HandleEvents(active_wnd);
